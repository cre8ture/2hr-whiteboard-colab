...
function Example() {
  ...
  const [textBoxes, setTextBoxes] = useState<TextBoxEvent[]>([]);
  ...
  useEventListener((eventData) => {
    const event = eventData.event as TextBoxEvent;
    setTextBoxes((textBoxes) => textBoxes.concat([event]));
  });


  // listens to the drawing event from the other user, then will display on this users svg
  useEventListener((eventData) => {
    const event = eventData.event as DrawingEvent;

    console.log("EVENT", event)
    // Check if event is an array of paths
    if (Array.isArray(event)) {
      const scaledPaths = event.map(path => ({
        x: path.x * window.innerWidth,
        y: path.y * window.innerHeight
      }));
      drawPath(scaledPaths, userId); // If it's already an array of paths, pass it directly
    } else {
      // If it's a single path object, wrap it in an array
      const scaledPath = {
        x: event.x, //* window.innerWidth,
        y: event.y //* window.innerHeight
      };
      drawPath([scaledPath], userId);
    }
  });
  ...
  // this updates this users drawings
  useEffect(() => {
    ...
    <>
      <div
        ...
        onPointerMove={(event) => {
          ...
          if (drawing && cursor) {
            console.log("i am cursor", cursor);
            setPath((path) => {
              const newPath = path.concat([{ // when i remove / windows.innerWidth it draws on my side but not
                // x: cursor.x, 
                // y: cursor.y
              ...
              broadcast(newPath); // Broadcast the new path
              return newPath;
            });
          }
        ...
        onPointerDown={(event) => {
          ...
          setDrawing(true); // Start drawing when pointer is down
        }}
        // Add this inside your onPointerDown event handler
        // onDoubleClick={(event) => {
        //   const x = Math.round(event.clientX);
        //   const y = Math.round(event.clientY);
        //   setTextBoxes((textBoxes) => textBoxes.concat([{ x, y, value: "" }]));
        // }}

        // Existing onPointerUp event handler
        onPointerUp={(event) => {
          setState((state) =>
            state.mode === CursorMode.Reaction
  ...
  <input
    ...
    onChange={(event) => {
      ...
      broadcast({ x: textBox.x, y: textBox.y, value });
    }}
  />
))} */}
